...
//     document = await logsCollection.findOne({});
...
//   socket.emit("documentData", document); // Assuming "document" contains the fetched MongoDB document
...
const io = require('socket.io')(2018, {
  cors: {
    origin: '*',
  },
});
...
io.on('connection', (socket) => {
  ...
  //   io.emit('documentData', data);
  //   console.log(data);
  // });

  io.emit('documentData', document);
});
